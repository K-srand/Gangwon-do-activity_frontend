...
function Report() {
    ...
    useEffect(() => {
        const fetchReportedData = async () => {
            try {
                const response = await axios.get(`http://223.130.138.174:4040/api/v1/report/?page=${currentPage - 1}&size=15`);
        ...
        if (window.confirm('삭제하시겠습니까?')) {
            try {
                await Promise.all(selectedReports.map(reportedContentNo => 
                    axios.delete(`http://223.130.138.174:4040/api/v1/report/delete/${reportedContentNo}`)
                ));
    ...
    const handleContentRestriction = async () => {
        if (window.confirm('해당 콘텐츠를 제재하시겠습니까?')) {
            try {
                await Promise.all(selectedReports.map(reportedContentNo => {
                    return axios.patch('http://223.130.138.174:4040/api/v1/admin/sanctioncontent', reportedContentNo, {
                        headers: {
    ...
    const handledesanctionContent = async () => {
        if (window.confirm('해당 콘텐츠를 제재 해제하시겠습니까?')) {
            try {
                await Promise.all(selectedReports.map(reportedContentNo => {
                    return axios.patch('http://223.130.138.174:4040/api/v1/admin/desanctioncontent', reportedContentNo, {
                        headers: {
                            'Content-Type': 'application/json'
                        }
                    });
