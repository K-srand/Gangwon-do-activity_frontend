...
function MainUpper({ token }) {
  ...
  useEffect(() => {
    // 추천 10개 액티비티 호출
    axios.get('http://223.130.138.174:4040/api/v1/getjson/getplace')
      .then(response => {
  ...
  useEffect(() => {
    const loadNaverMapScript = async () => {
      if (!window.naver) {
        try {
          const response = await axios.get('http://223.130.138.174:4040/api/v1/getmap');
    ...
    if (locations.length > 0 && window.naver && mapInitialized) {
      ...
      locations.forEach(location => {
        ...
        naver.maps.Event.addListener(marker, 'click', () => {
          axios.post('http://223.130.138.174:4040/api/v1/getjson/getplacetitle', {
      ...
      // 강원도 행정구역 데이터 레이어 호출
      axios.get('http://223.130.138.174:4040/resources/json/gangwondo.json')
        .then(response => {
  ...
  const category = (value) => {
    axios.post('http://223.130.138.174:4040/api/v1/getjson/getplacecat', {
      placeCat: value
    })
      .then(response => {
  ...
  const favoriteplace = (item) => {
    if (userId) {
      axios.post('http://223.130.138.174:4040/api/v1/getmyfavorite', {
        placeNo: item.id,
        userId: userId
      })
        .then(response => {
