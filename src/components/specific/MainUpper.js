...
function MainUpper({ token }) {
  ...
  // 로그인 여부 확인
  useEffect(() => {
    axios.get('http://172.31.3.146:4040/api/v1/user', {
      headers: {
  ...
  useEffect(() => {
    // 추천 10개 액티비티 호출
    axios.get('http://172.31.3.146:4040/api/v1/getjson/getplace')
      .then(response => {
    ...
    if (locations.length > 0 && window.naver && mapInitialized) {
      ...
      locations.forEach(location => {
        ...
        naver.maps.Event.addListener(marker, 'click', () => {
          axios.post('http://172.31.3.146:4040/api/v1/getjson/getplacetitle', {
      ...
      // 강원도 행정구역 데이터 레이어 호출
      axios.get('http://172.31.3.146:4040/resources/json/gangwondo.json')
        .then(response => {
  ...
  const category = (value) => {
    axios.post('http://172.31.3.146:4040/api/v1/getjson/getplacecat', {
      placeCat: value
    })
      .then(response => {
  ...
  const favoriteplace = (item) => {
    if (userId) {
      axios.post('http://172.31.3.146:4040/api/v1/getmyfavorite', {
        placeNo: item.id,
        userId: userId
      })
        .then(response => {
  ...
  const fetchFirstData = async () => {
      try {
          const firstResponse = await axios.post('http://172.31.3.146:4040/api/v1/recommend', {});
  ...
  const fetchSecondData = async () => {
      try {
          ...
          for (let i = 0; i < myCourseNo.length; i++) {
              ...
              const secondResponse = await axios.get(`http://172.31.3.146:4040/api/v1/recommend/${courseNo}`);
