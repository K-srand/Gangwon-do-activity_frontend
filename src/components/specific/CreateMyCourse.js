...
const CreateMyCourse = ({ token }) => {
    ...
    const handleSave = (saveSelectedImages) => {
        ...
        if (imagesToUse.length === 4 && window.naver) {
            ...
            // 경로 데이터 초기화 및 필요한 경로 데이터만 사용
            const pathCoordinates = [];
            for (let i = 0; i < pathData.current.length; i++) {
                pathCoordinates.push(...pathData.current[i].map(point => new naver.maps.LatLng(point[1], point[0])));
            }
    ...
    const calculateDurations = (images) => {
        ...
        // 인덱스 1번과 4번을 연결하지 않도록 마지막 이미지는 무시합니다
