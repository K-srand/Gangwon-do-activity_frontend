...
function Community() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get(`https://3.36.27.202:4040/api/v1/board/?page=${currentPage -1}&size=6`);
    ...
    const fetchBestPosts = async () => { // 주간 베스트 게시글을 가져오는 함수 추가
      try {
        const response = await axios.get('https://3.36.27.202:4040/api/v1/board/best');
  ...
  const fetchNoticePosts = async () => { // 공지글 가져오는 함수
    try {
      const response = await axios.get('https://3.36.27.202:4040/api/v1/board/notice');
