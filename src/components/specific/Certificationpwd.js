...
function CertificationPwd({ closeModal, setIsEmailVerified }) {
  ...
  const handleCheck = async (e) => {
    ...
    try {
      const response = await axios.post('http://223.130.138.174:4040/api/v1/auth/check-certification', {
        userId,
        email,
        certificationNumber
      }, {
  ...
  return (
    <div className='certification-modal-main'>
      ...
      <div className='certification-modal-main-check-textbox'>
        <div className='bold'>임시 비밀번호 발급 인증 메일</div>
        <div className='detail'>본 이메일은 비밀번호 찾기를 위한 필수 사항입니다.</div>
      </div>
...
export default CertificationPwd;
