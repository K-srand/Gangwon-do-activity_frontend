...
function AllUserList() {
    ...
    useEffect(() => {
        ...
        fetch(`http://3.36.27.202:4040/api/v1/admin/getuserlist?page=${currentPage - 1}&size=15`)
        ...
        if (window.confirm('해당 유저를 제재하시겠습니까?')) {
            try {
                await Promise.all(selectedUsers.map(userNo =>
                    fetch('http://3.36.27.202:4040/api/v1/admin/sanction', {
                        method: 'PATCH',
                        headers: {
        ...
        if (window.confirm('선택된 유저를 탈퇴 해제하시겠습니까?')) {
            try {
                await Promise.all(selectedUsers.map(userNo => {
                    ...
                    return fetch('http://3.36.27.202:4040/api/v1/admin/reuser', {
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json',
                        },
    ...
    const handleDesanction = async () => {
        if (window.confirm('선택된 유저를 제재 해제하시겠습니까?')) {
            try {
                await Promise.all(selectedUsers.map(userNo =>
                    fetch('http://3.36.27.202:4040/api/v1/admin/desanction', {
                        method: 'PATCH',
                        headers: {
                            'Content-Type': 'application/json',
                        },
